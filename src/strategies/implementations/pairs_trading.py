...
class PairsTradingStrategy(BaseStrategy):
    ...
    async def evaluate(self) -> list[Signal]:
        ...
        # OLS hedge ratio (beta): regress A on B
        closes_a_arr = np.array(closes_a, dtype=float)
        closes_b_arr = np.array(closes_b, dtype=float)
        # Simple OLS: beta = cov(A,B) / var(B)
        cov_ab = np.cov(closes_a_arr, closes_b_arr)[0, 1]
        var_b = np.var(closes_b_arr, ddof=1)
        if var_b == 0:
            return signals
        ...
        z_score = (current_spread - np.mean(spreads)) / spread_std
        ...
        elif abs(z_score) < self._exit_z and self._position_open:
